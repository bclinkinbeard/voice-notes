import { RelayStoreError } from './sync-store.js';
...
function matchVaultRoute(pathname) {
  const match = pathname.match(/^\/v1\/(?:vaults|sync)\/([^/]+)\/(events|artifacts)$/);
  if (!match) return null;
  return { vaultId: decodeURIComponent(match[1]), resource: match[2] };
}
...
export function createSyncRequestHandler(store) {
  const legacyHandler = createRelayRequestHandler(store);
  ...
  return async function syncRequestHandler(req, res) {
    ...
    try {
      ...
      if (req.method === 'POST' && route.resource === 'events') {
        ...
        writeJson(res, 200, await store.upsertEvents(route.vaultId, auth, payload.events || []));
        return;
      }
      if (req.method === 'GET' && route.resource === 'events') {
        const result = await store.readEvents(route.vaultId, auth, url.searchParams.get('since') || '');
        writeJson(res, 200, { cursor: result.cursor, events: result.records });
        return;
      }
      if (req.method === 'POST' && route.resource === 'artifacts') {
        const payload = await readJson(req);
        writeJson(res, 200, await store.upsertArtifacts(route.vaultId, auth, payload.artifacts || []));
        return;
      }
      if (req.method === 'GET' && route.resource === 'artifacts') {
        ...
        writeJson(res, 200, { cursor: result.cursor, artifacts: result.records });
        return;
      }
      ...
      writeJson(res, 500, { error: error.message || 'Unexpected sync error.' });
    }
  };
}
