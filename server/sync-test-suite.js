...
export async function runSyncBehaviorSuite(label, withSync) {
  const { assertEqual, assertDeepEqual, counts } = makeAssertions();

  await suite(`${label}: Sync service round-trip uses vaultKey for payload crypto`, async () => {
    await withSync(async (baseUrl) => {
      const vault = {
        id: 'vault:shared',
        syncUrl: baseUrl,
      ...
      assertDeepEqual(pulledEvents.events, [event], 'round-trips event payloads through the sync service');
  ...
  await suite(`${label}: Sync service rejects bad auth, dedupes, and preserves ciphertext`, async () => {
    await withSync(async (baseUrl) => {
  ...
  await suite(`${label}: Sync cursors still return late-arriving older history`, async () => {
    await withSync(async (baseUrl) => {
      const vault = {
        id: 'vault:cursor',
        syncUrl: baseUrl,
  ...
  await suite(`${label}: Empty vault sync initializes the remote sync service`, async () => {
    await withSync(async (baseUrl) => {
      const vault = {
        id: 'vault:empty',
        syncUrl: baseUrl,
      ...
      assertEqual(pushResult.accepted, 0, 'empty event push still initializes the remote vault');
      assertEqual(pushArtifactResult.accepted, 0, 'empty artifact push still initializes the remote vault');
